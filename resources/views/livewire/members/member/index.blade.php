<div>
            ...
            <div class="flex flex-col md:flex-row justify-between items-start gap-4 md:gap-0">
                ...
                <div class="flex bg-gray-100 p-1 rounded-lg border border-gray-200 gap-1">
                    ...
                    <button wire:click="switchView('list')"
                        class="cursor-pointer px-3 py-2 text-xs font-medium rounded-md transition-all {{ $viewType === 'list' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                        List
                    </button>

                    {{-- 2. Chart View --}}
                    <button wire:click="switchView('chart')"
                        class="cursor-pointer px-3 py-2 text-xs font-medium rounded-md transition-all {{ $viewType === 'chart' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                        Chart
                    </button>

                    {{-- 3. Tree View --}}
                    <button wire:click="switchView('tree')"
                        class="cursor-pointer px-3 py-2 text-xs font-medium rounded-md transition-all {{ $viewType === 'tree' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
            ...
            <div class="py-4 md:py-6">
                ...
                <div class="border-b p-4 shadow-sm rounded-md mb-4 bg-white">
                    ...
                    <div class="grid grid-cols-1 gap-2 mb-4">
                        <div>
                            {{-- NIK: Disabled saat Edit --}}
                            <x-modal.input name="nik" label="NIK <span class='text-red-500'>*</span>" type="text"
                                placeholder="Contoh: 1234567890" required :disabled="$member_id ? true : false" />
                        </div>
                    </div>
    ...
    {{-- MODAL DETAIL CARD --}}
    @if ($isCardOpen)
        <x-modal.card-modal :formTitle="'Detail'" action="closeCardModal()">
            <div class="py-4 md:py-6">
                {{-- PROFILE CARD --}}
                <div class="max-w-2xl mx-auto bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-lg overflow-hidden max-h-[700px] overflow-y-auto">
                    {{-- HEADER BACKGROUND --}}
                    ...
                    <div class="px-6 pb-6 space-y-4">
                        ...
                        <div class="bg-white rounded-lg p-4 border border-gray-200">
                            <h3 class="font-semibold text-gray-900 mb-3 text-sm uppercase tracking-wider">Personal Information</h3>
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div><p class="text-gray-500 font-medium">NIK</p><p class="text-gray-900 font-semibold">{{ $nik }}</p></div>
                                <div><p class="text-gray-500 font-medium">Phone</p><p class="text-gray-900 font-semibold">{{ $phone_number }}</p></div>
                                <div><p class="text-gray-500 font-medium">Gender</p><p class="text-gray-900 font-semibold capitalize">{{ $gender }}</p></div>
                                <div><p class="text-gray-500 font-medium">Birth Date</p><p class="text-gray-900 font-semibold">{{ $birth_date }}</p></div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-lg p-4 border border-gray-200">
                            ...
                            <div class="space-y-2 text-sm">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div><p class="text-gray-500 font-medium">Province</p><p class="text-gray-900">{{ optional($province)->name }}</p></div>
                                    <div><p class="text-gray-500 font-medium">Domicile</p><p class="text-gray-900">{{ optional($domicile)->name }}</p></div>
                                </div>
                            </div>
                        </div>
                        ...
                        {{-- BANK INFO SECTION --}}
                        <div class="bg-white rounded-lg p-4 border border-gray-200">
                            <h3 class="font-semibold text-gray-900 mb-3 text-sm uppercase tracking-wider">Bank Information</h3>
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div><p class="text-gray-500 font-medium">Bank</p><p class="text-gray-900 font-semibold">{{ $bank_name }}</p></div>
                                <div><p class="text-gray-500 font-medium">Account No</p><p class="text-gray-900 font-semibold">{{ $account_number }}</p></div>
                                <div class="col-span-2"><p class="text-gray-500 font-medium">Account Name</p><p class="text-gray-900 font-semibold">{{ $account_name }}</p></div>
                                @if ($npwp)
                                    <div class="col-span-2"><p class="text-gray-500 font-medium">NPWP</p><p class="text-gray-900 font-semibold">{{ $npwp }}</p></div>
                                @endif
                            </div>
                        </div>
                    </div>
