        ...
        <x-modal.form-modal :formTitle="$member_id ? 'Edit Member' : 'Add Member'" action="store()">
            <div class="py-4 md:py-6">
                ...
                <div class="border-b p-4 shadow-sm rounded-md mb-4 bg-white">
                    ...
                    <div>
                        <x-modal.searchable-select name="parent_user_id" label="Parent User" wire:model="parent_user_id"
                            {{-- GANTI $allMembers JADI $parentOptions --}} :options="($parentOptions ?? collect([]))
                                ->map(
                                    fn($m) => [
                                        'value' => $m->user->id,
